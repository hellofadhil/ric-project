...
namespace OnePro.Front.Mappers
{
    public static class RicMapper
    {
        ...
        private static async Task<(List<string> asIs, List<string> toBe, List<string> expected)> SaveAllFilesAsync(
            RicCreateViewModel model,
            Func<IEnumerable<IFormFile>?, Task<List<string>>> saveFilesAsync
        )
        {
            ...
            var expected = await SaveOrEmptyAsync(() => saveFilesAsync(model.ExpectedCompletionFiles));
        ...
        private static List<string>? PreferNewOrExisting(List<string> newUrls, List<string>? existingUrls)
